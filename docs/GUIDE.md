...
| `getVersion()` | `"1.0.2-dev"` on dev branch · `"1.0.2"` stable (Semver string, no envelope) |
...
- `Access to this API has been restricted` on `/.dockerenv` (or `C:\.dockerenv`): MongoDB driver container probe vs FiveM FS sandbox — fixed by `src/fivemFsCompat.ts` (rebuild + restart). Not solved by server.cfg allowlists alone on older FXServer artifacts.
